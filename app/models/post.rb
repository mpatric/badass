class Post < ActiveRecord::Base
  ...
  scope :published, -> { where('version IS NOT NULL') }
  scope :unpublished, -> { where('version IS NULL') }
  scope :outdated, -> { where('version IS NOT NULL AND version != version_draft') }
  scope :current, -> { where('version IS NOT NULL AND version = version_draft') }
  scope :for_author, ->(user) { where('user_id = ?', user.id) }
  scope :with_tag, ->(tag) { joins('JOIN posts_tags pt ON pt.post_id = posts.id').where('pt.tag_id = ?', tag.id) }
  scope :since, ->(date) { where('last_published_at > ?', date) }
